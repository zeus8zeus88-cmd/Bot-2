...
async def help_slash(interaction: discord.Interaction):
    ...
    embed.add_field(name="**▸ تسجيل شغل جديد**",
                    value="`!تحليل` أو `/تسجيل`\n*يدعم فصلاً واحداً أو عدة فصول، وأنماط أنواع مثل `ترجمة_كوري-تحرير`*",
                    inline=False)
...
# ----------------------------------------------------------------------
# Slash command: تسجيل (RegisterModal)
# ----------------------------------------------------------------------
class RegisterModal(discord.ui.Modal, title="تسجيل شغل جديد"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        work = await get_work(self.work_name.value)
        if not work:
            await interaction.response.send_message(f"❌ العمل `{self.work_name.value}` غير موجود في قائمة الأعمال المدفوعة. تواصل مع الإدارة.", ephemeral=True)
            return
        if not work.get("active", True):
            await interaction.response.send_message(f"❌ العمل `{self.work_name.value}` معطل حالياً.", ephemeral=True)
            return
        ...
        original_types = parse_mixed_types(self.types.value, len(chapters_list))
        ...
        for idx, ch in enumerate(chapters_list):
            if ch in kept_set:
                filtered_types.append(original_types[idx])

        for t in filtered_types:
            if t not in PRICES:
                await interaction.response.send_message(f"❌ النوع `{t}` غير صحيح. الأنواع: {', '.join(PRICES.keys())}", ephemeral=True)
                return
        ...
        added = 0
        username = interaction.user.name  # store the unique username
        ...
        if notify_channel_id:
            ...
            if channel:
                await channel.send(f"📢 {interaction.user.mention} أضاف {added} فصول مدفوعة في عمل `{self.work_name.value}`")
...
async def analysis(ctx, *, text=None):
    ...
    added = 0
    username = ctx.author.name  # store the unique username
    for idx, ch in enumerate(paid_chapters):
        work_type = filtered_types[idx]
        total = PRICES[work_type]
        records[user_id].append({
...
class WorkDetailsView(discord.ui.View):
    def __init__(self, work_name, chapters_list, user_id, user_name, currency, back_callback: callable = None):
        ...
        self.user_id = user_id
        self.user_name = user_name  # display name (already formatted)
...
# ----------------------------------------------------------------------
# /الأعمال command (replaces /مشاريع)
# ----------------------------------------------------------------------
async def get_works_info(guild: discord.Guild):
    ...
    for w in approved_works:
        ...
        for uid_str, count in contributors.items():
            uid = int(uid_str)
            # Get the stored username from any record of this user (they may have multiple)
            username_hint = None
...
class MemberSelect(discord.ui.Select):
    def __init__(self, work_name, members_info, guild, works_info_callback=None):
        self.work_name = work_name
        self.members_info = members_info  # list of (user_id, display_name)
        self.guild = guild
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # get the display name we already computed
...
# ----------------------------------------------------------------------
# Stats command (with member display)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# My works / member works commands (unchanged, but uses stored username in display)
# ----------------------------------------------------------------------
@bot.tree.command(name="أعمالي", description="عرض أعمالك مجمعة")
@app_commands.checks.cooldown(1, 5, key=lambda i: (i.user.id, i.command.qualified_name))
...
@bot.tree.command(name="شغل", description="عرض شغل عضو مجمّع")
@app_commands.checks.cooldown(1, 5, key=lambda i: (i.user.id, i.command.qualified_name))
...
# ----------------------------------------------------------------------
# Other admin commands (dashboard, audit, report, edit, export, settings)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Works management commands (add, delete, edit, list)
# ----------------------------------------------------------------------
...
class WorksListPaginator(discord.ui.View):
    def __init__(self, works: list):
        ...
        self.works = works
        self.current_page = 0
        self.per_page = 20  # 20 fields per page (safe margin under 25)
